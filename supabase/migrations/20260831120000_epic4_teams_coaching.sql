-- Epic 4 — Teams & Coaching: trial-start trigger (4.1), team_has_access (4.2),
-- team_flagged_athletes (4.4).
--
-- Everything else 4.1/4.2/4.4/4.7/4.11 ask for at the RLS layer
-- (has_coach_access reading trial_length_days from the row, team_members
-- admin-only update/delete, and every coach-read policy's consent gate) was
-- already added pre-emptively in 1.7's row_level_security.sql — confirmed by
-- reading it, not re-added here.

-- start_trial_for_creator ----------------------------------------------------
-- Runs as security definer: billing_status has no insert policy for
-- `authenticated` at all (row_level_security.sql), so a plain trigger running
-- as the inserting user's role couldn't write the row itself.
create function private.start_trial_for_creator () returns trigger language plpgsql security definer
set search_path = '' as $$
begin
  if not exists (select 1 from public.billing_status where profile_id = new.created_by) then
    insert into public.billing_status (profile_id, trial_started_at, trial_length_days)
    values (
      new.created_by,
      now(),
      -- Deterministic 50/50 split — control=7 days, variant_b=5 days (locked
      -- decision, CODING_PLAN.md). Computed here, not client-supplied.
      case when abs(hashtext(new.created_by::text)) % 2 = 0 then 7 else 5 end
    );
  end if;
  -- A profile that already has a billing_status row (mid-trial, expired, or
  -- subscribed) is left untouched — a second team never resets or extends
  -- a trial.
  return new;
end;
$$;

create trigger trg_start_trial_for_creator
after insert on public.teams for each row
execute function private.start_trial_for_creator ();

-- team_has_access -------------------------------------------------------------
-- Deliberately public (unlike is_team_member/is_team_admin/has_coach_access,
-- which were moved to `private` in 1.7's follow-up migration specifically to
-- remove their RPC surface) — 4.8 and 4.10 need this callable directly from
-- the client, not just composed into other policies.
--
-- `bool_or` across a team's admins rather than "the" admin: 4.7 allows
-- promoting more than one member to admin, so a team can have more than one
-- — access is active if any of them currently has it.
create function public.team_has_access (p_team_id uuid) returns boolean language sql stable security definer
set search_path = '' as $$
  select coalesce(bool_or (private.has_coach_access (tm.profile_id)), false)
  from public.team_members tm
  where tm.team_id = p_team_id and tm.role = 'admin';
$$;

-- team_flagged_athletes -------------------------------------------------------
-- security invoker (the default) — every personal-data table this reads
-- already carries its own coach-read RLS policy (admin + active access +
-- athlete consent, all from 1.7), so a caller without access or without a
-- consenting relationship to a given athlete simply sees no rows from that
-- athlete for the tables backing pain/below_baseline/rising_load, which
-- naturally drops them out of the final `where` filter below. No need to
-- re-derive that logic here.
...
set search_path = '' as $$
  ...
  -- alone never flags (locked decision, CODING_PLAN.md).
  pain as (
    select
      roster.profile_id,
      max(
    ...
    from roster
      left join public.body_map_entries bme on bme.profile_id = roster.profile_id
